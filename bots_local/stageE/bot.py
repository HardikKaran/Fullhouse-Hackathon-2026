...
USE_STAGE_D = False
# USE_STAGE_E: generalization fix exposed by the diverse villain pool
# (PLAN §E / results/stageE_gauntlet.log). The old `station_field` logic sized
# value bets SMALL (assuming "stations" fold to big bets — true only of the
# reference pot-odds FOLDERS, which classify as tag/nit, not station). Against a
# TRUE never-fold calling station that cost us chips vs even Tier-1 (-900 paired).
# Fix: a genuinely sticky station (low fold_freq) calls ANY size, so OVERBET for
# value; a "calls-small / folds-big" caller keeps the sub-fold-threshold size.
# False => Stage-C station sizing.
USE_STAGE_E = True
...
def _exploits(state, model, live_ids, hero_seat):
    ...
    # vs a station field: value-bet THIN (they call wide) and never bluff (fe gate
    # already kills it, but be explicit). SIZING depends on whether they actually
    # fold to big bets:
    if station_field:
        ex["sb_freq"] = 0.0          # never bluff a station — it doesn't fold
        ex["allow_sb_raise"] = False
        st_fold = min((p["fold_freq"] for p in profs
                       if p["archetype"] == "station"), default=0.4)
        if USE_STAGE_E and st_fold < 0.35:
            # TRUE never-fold calling station. Diverse-pool finding
            # (results/stageE_fix.log): BOTH shrinking sizing (the old Stage-C
            # `overbet_size 0.5` under-extracts from monsters) AND overbetting thin
            # value (value_th 0.50 @ 0.9 pot — its wide range calls with better and
            # we lose big) lost vs Tier-1's plain NEUTRAL sizing. So keep NEUTRAL
            # sizing exactly (value 0.6 / overbet 0.9 / value_th 0.56) and only
            # suppress the dead bluffs (set above) — provably non-worse than Tier-1.
            pass
        else:
            # "calls small / folds big" caller — size value UNDER its fold
            # threshold so it still pays (don't overbet it out of the pot).
            ex["value_th"] = 0.50
            ex["value_size"] = 0.5
            ex["overbet_size"] = 0.5
            ex["raise_th"] = 0.62
