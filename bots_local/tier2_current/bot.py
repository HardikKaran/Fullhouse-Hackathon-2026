...
def _preflop(state, hole, model):
    ...
    if not raised:
        ...
        # Tier-2 steal: widen the open vs a field that folds a lot (nits), keep
        # it tight vs stations/maniacs who don't fold to steals. Bounded ±1.0.
        if USE_RANGE_MODEL:
            open_th -= _steal_loosen(state, model)
        if chen >= open_th:
...
def _steal_loosen(state, model):
    """How much to lower the open threshold (widen steals). Positive only vs a
    field that folds a lot; 0 vs stations/maniacs. Bounded to 1.0 Chen points."""
    ids = _live_opp_ids(state)
    if not ids:
        return 0.0
    # Only steal wider when EVERY live opponent left to act folds readily.
    fold = 1.0
    any_known = False
    for bid in ids:
        ...
        fold = min(fold, p["fold_freq"])
    if not any_known:
        return 0.0
    return max(0.0, min(1.0, (fold - 0.45) * 3.0))
